...
// vib3 K26 MLA variant — derived from mla_sm120.cu. See NOTICE.md.
// Key deltas vs upstream FP8 path:
//   * HEAD_GRP_SIZE = 64 (instead of 128) for K2.6.
//   * FP16 inputs + FP16 KV cache (instead of __nv_fp8_e4m3).
//   * MMA atom: m16n8k16.f16 (instead of m16n8k32.e4m3).
//   * X softmax buffer: stored as FP16, not quantized to FP8.
//   * computeRowSumFromF8 disabled — falls back to FP32 row-sum.
//   * partElemsV halved (128 -> 64) so V tensor-map swizzle stays <= 128 B.
//   * nbXVBufs forced to 1 so per-block smem fits the sm_120a 99 KB budget.
...
// Override qmmaShape: FP16 uses m16n8k16 (k=16 elems), not FP8's m16n8k32.
// Register packing is the same (4 b32 for A, 2 b32 for B per thread) — only
// the K-tile element count differs, so downstream loop bounds and swizzle
// math re-compute correctly when qmmaShape.k becomes 16.
namespace vib3_k26 {
inline constexpr mmaShape qmmaShape_k26 = {16, 8, 16};
}
#define qmmaShape ::vib3_k26::qmmaShape_k26
...
// K26: halved from upstream 64 so V-buffer (tokensPerTile * gemm1V * sizeof(half))
// fits an XV smem budget that is a divisor of warpTile.y (= 16 under K26).
// WarpOutSwizzleBuf::rows must divide Dst::size (= warpTile.y = 16); with
// tokensPerTile=32 the XVBuffer rounds to 32 KB (= 16 rows × 2 KB), which
// divides 16 cleanly.
inline constexpr uint32_t tokensPerTile = 32;
// K26: halved from upstream 128 so partElemsV * sizeof(half) = 128 B, which
// keeps the V tensor-map swizzle at the supported 128 B.
inline constexpr uint32_t partElemsV = 64;
...
using MathElem = CacheElem;  // half under K26
...
// K26: FP8 e4m3 scales collapse to 1 — softmax output stays FP16 in smem.
inline constexpr float xScale = 1.f;
__constant__ constexpr float rcpXScale = 1.f;

inline constexpr uint32_t nbRegsForIOWarps = 32;
inline constexpr uint32_t nbRegsForMathWarps = 232;

// K26: FP8-tensor-core row-sum optimization doesn't apply to FP16.
inline constexpr bool computeRowSumFromF8 = false;
...
struct SharedMemA {
  // K26: halved from upstream 12 — FP16 ShmKPart is 2x the byte size of FP8
  // (16 KB vs 8 KB each after partElemsK=64, tokensPerTile=64 under FP16), so
  // 6 buffers takes ~48 KB instead of 96 KB, keeping SharedMemA well under the
  // sm_120a 99 KB budget. Pipeline depth loss is acceptable on this path.
  // Note: actual ShmKPart size is still (tokensPerTile * mathElemBytes) = 8 KB
  // under FP16 because grainsPerPartK already scales with mathElemBytes.
  static inline constexpr uint32_t nbKBufs = 6;
...
struct SharedMemB {
  // K26: single-buffer XV. Upstream uses 2 for pipelining; FP16 VBuffer is
  // 2x the byte size of FP8 so we trade double-buffering for smem headroom
  // to stay under sm_120a's 99 KB limit.
  static inline constexpr uint32_t nbXVBufs = 1;
  ...
  // K26: size XVBuffer to accommodate FP16 VBuffer+CgaXBuffer, rounded up to
  // a multiple of sizeof(OutputElem)*warpTile.x(128)*nbMathWarps(8) = 16384
  // so WarpOutSwizzleBuf::rows stays a multiple of 8 (see Consumer).
  static constexpr uint32_t xvBufTargetSize =
      roundUp<uint32_t>(mha::max<uint32_t>(sizeof(VBuffer) + sizeof(CgaXBuffer),
                                           headGrpSize * 128 * 2),
                        16384u);
  struct XVBuffer {
    VBuffer v;
    CgaXBuffer x;
    uint8_t pad[xvBufTargetSize - sizeof(VBuffer) - sizeof(CgaXBuffer)];
  };
  ...
  __device__ inline void invalidateBarriers(uint32_t thrdIdx) {
    // K26: barriers between xBars and isLastSubSeq:
    //   xBars[nbXBufs]*2 + vBars[nbVBufs]*2 + cgaXBufProduced[nbProducerCtasPerCga]
    //   + mathWarpsBar + multiBlockBars[nbMultiBlockBufs]*2.
    constexpr uint32_t nbBars =
        2 * nbXBufs + 2 * nbVBufs + nbProducerCtasPerCga + 1 + 2 * nbMultiBlockBufs;
#ifndef __CUDACC_RTC__
    constexpr uint32_t nbBarsRef =
...
#pragma unroll
          for (uint32_t idxAtomBx2 = 0; idxAtomBx2 < tileNbAtomBx2; idxAtomBx2++) {
            // K26: upstream FP8 had tileNbAtomBx2=4 and prefetched Q at the
            // midpoint (idxAtomBx2==2). Under K26 tileNbAtomBx2=2, so that
            // branch would never fire and regQBuf[idxInstKPrefetch] would
            // stay uninitialized across idxInstK > 0. Fire at the last
            // iteration instead — still earlier than the next idxInstK's MMA.
            if (idxAtomBx2 + 1 == tileNbAtomBx2 && prefetch) {
...
#pragma unroll
              for (uint32_t j = 0; j < 2; j++) {
                mma<half>(reinterpret_cast<float(&)[2][2]>(acc(i, 2 * idxAtomBx2 + j)),
...
#pragma unroll
      ...
      // K26: downcast softmax output float -> half with full FP16 range
      // (no e4m3 quantization); xScale / rcpXScale collapse to 1.
      // Each cell holds 4 uint32 = 8 halves = one stmatrix.b16.x4 grain.
      // Slot order must match stmatrix.b16.x4 r[0..3] = {Mat0, Mat1, Mat2, Mat3}
      // where the matIdx=L/8 partition places Mat0=TL, Mat1=BL, Mat2=TR, Mat3=BR
      // within each 16x16 block (the Mat16x32 quadrant convention used by
      // upstream's ldmatrix addressing — NOT CUTLASS's TL/TR/BL/BR order).
      //   m=0, col=j   -> TL -> xF16[0]
      //   m=1, col=j   -> BL -> xF16[1]
      //   m=0, col=j+1 -> TR -> xF16[2]
      //   m=1, col=j+1 -> BR -> xF16[3]
      Array2D<LdGrain, WarpAcc::rows, exactDiv(WarpAcc::cols, 2)> xF16;
#pragma unroll
      for (uint32_t i = 0; i < WarpAcc::rows; i++) {
#pragma unroll
        for (uint32_t j = 0; j < WarpAcc::cols; j += 2) {
          half2 const mat0 = __float22half2_rn(
              float2{xF32(i, j)(0, 0), xF32(i, j)(0, 1)});
          half2 const mat1 = __float22half2_rn(
              float2{xF32(i, j)(1, 0), xF32(i, j)(1, 1)});
          half2 const mat2 = __float22half2_rn(
              float2{xF32(i, j + 1)(0, 0), xF32(i, j + 1)(0, 1)});
          half2 const mat3 = __float22half2_rn(
              float2{xF32(i, j + 1)(1, 0), xF32(i, j + 1)(1, 1)});
          xF16(i, j / 2)[0] = reinterpret_cast<uint32_t const&>(mat0);
          xF16(i, j / 2)[1] = reinterpret_cast<uint32_t const&>(mat1);
          xF16(i, j / 2)[2] = reinterpret_cast<uint32_t const&>(mat2);
          xF16(i, j / 2)[3] = reinterpret_cast<uint32_t const&>(mat3);
        }
      }
      // K26: use FP32 row-sum (FP8 tensor-core path doesn't apply).
      ThrdRegRowMax const rowSum =
          computeRowSumF32<warpTile.y, warpTile.x>(this_warp(), xF32);

      // store X and rowSum into L2 scratch buffer
      if (!skipXBarWait) {
      ...
      storeOrderedXToShm(smem.x.x, xF16, tileBaseRow, lane);
  ...
  __device__ inline void storeOrderedXToShm(
      XBuffer& dst,
      Array2D<LdGrain, WarpAcc::rows, exactDiv(WarpAcc::cols, 2)> const& src,
...
__device__ inline void Producer::storeOrderedXToShm(
  ...
  // K26: FP16 path. Write each 16x16 half block via stmatrix.b16.x4.
  // Matrix-order follows matIdx=L/8 (same as upstream Mat16x32Loader): lanes
  // 0..7 -> Mat0 = TL, 8..15 -> Mat1 = BL, 16..23 -> Mat2 = TR, 24..31 -> Mat3 = BR
  // (i.e. column-major 2x2 quadrant tiling, NOT CUTLASS's row-major order).
  // The src packing above places TL/BL/TR/BR in xF16[0..3] to match.
  uint32_t const matIdx = lane / 8;
  uint32_t const rowInMat = lane % 8;
  uint32_t const matRowBase = 8 * (matIdx & 1U);   // 0 for Mat0/Mat2, 8 for Mat1/Mat3
  uint32_t const matColLdGrain = matIdx >> 1;      // 0 for Mat0/Mat1, 1 for Mat2/Mat3
  using Src = mha::decay_t<decltype(src)>;
#pragma unroll
  for (uint32_t idxInstK = 0; idxInstK < Src::cols; idxInstK++) {
#pragma unroll
    for (uint32_t idxInstM = 0; idxInstM < Src::rows; idxInstM++) {
      uint32_t const row = tileBaseRow + idxInstM * 16 + matRowBase + rowInMat;
      uint32_t const colGrain = idxInstK * 2 + matColLdGrain;
      auto p = const_cast<LdGrain*>(&dst.template at<true>(row, colGrain));
      stmatrix<false, 4>(p, src(idxInstM, idxInstK));
    }
  }
}
...
__device__ inline void Consumer::compute() {
  ...
  // K26: rB/cB not needed — V load uses its own (rV, cV) derivation below.
...
#pragma unroll
    ...
    // K26: iterate across V-parts that cover this warp's N-range. Under
    // FP16 partElemsV is halved (64), so one warp tile spans 2 V-parts.
    // Upstream FP8 had partElemsV == warpTile.x (one part per tile).
    constexpr uint32_t nbVPartsInWarpTile = exactDiv(warpTile.x, partElemsV);
    constexpr uint32_t atomBx2PerVPart = exactDiv(partElemsV, qmmaShape.n * 2);
    static_assert(atomBx2PerVPart * nbVPartsInWarpTile == warpTileNbAtomBx2);
...
#pragma unroll
      for (uint32_t idxVPart = 0; idxVPart < nbVPartsInWarpTile; idxVPart++) {
        auto const& vBufPart = smem.v(idxVBuf)[tileIdx.x * nbVPartsInWarpTile + idxVPart];
#pragma unroll
        for (uint32_t idxAtomBx2 = 0; idxAtomBx2 < atomBx2PerVPart; idxAtomBx2++) {
          // ldmatrix.x4.trans.b16: addresses use Mat16x32Loader's (row=L%16,
          // col=L/16) convention over a 16x16 half block. The 4 returned
          // b32s correspond to Mat0=(K[0..7],N[0..7]), Mat1=(K[0..7],N[8..15]),
          // Mat2=(K[8..15],N[0..7]), Mat3=(K[8..15],N[8..15]). Pairing
          // Mat0+Mat2 yields a K=16 B operand for N[0..7] (first AtomB);
          // Mat1+Mat3 yields the N[8..15] B operand (second AtomB).
          uint32_t const rV = lane % 16;
          uint32_t const cV = lane / 16;
          auto const* vPtr = &vBufPart.template at<true>(
              qmmaShape.k * idxInstK + rV, idxAtomBx2 * 2 + cV);
          auto const data = ldmatrix<true, 4>(vPtr);
          // K26: ldmatrix.x4.trans.b16 with Mat16x32-convention addressing
          // (lane L at row=L%16, col=L/16 LdGrain) yields 4 matrices ordered
          //   data[0] = Mat0 = src rows 0..7, cols 0..7   (trans -> k=0..7, n=0..7)
          //   data[1] = Mat1 = src rows 8..15, cols 0..7  (trans -> k=8..15, n=0..7)
          //   data[2] = Mat2 = src rows 0..7, cols 8..15  (trans -> k=0..7, n=8..15)
          //   data[3] = Mat3 = src rows 8..15, cols 8..15 (trans -> k=8..15, n=8..15)
          // For m16n8k16.f16 B operands (k=16 x n=8 per MMA), pair along k:
          //   v[0] for n=0..7  = {Mat0, Mat1} = {data[0], data[1]}
          //   v[1] for n=8..15 = {Mat2, Mat3} = {data[2], data[3]}
          AtomB const v[2] = {data[0], data[1], data[2], data[3]};
          uint32_t const atomIdxWarp = idxVPart * atomBx2PerVPart + idxAtomBx2;
#pragma unroll
          for (uint32_t i = 0; i < WarpAcc::rows; i++) {
#pragma unroll
            for (uint32_t j = 0; j < 2; j++) {
              mma<half>(
                  reinterpret_cast<float(&)[2][2]>(acc(i, 2 * atomIdxWarp + j)),
...
#pragma unroll
        ...
        // K26: OutputHead is half (not bfloat16), convert accordingly.
        reinterpret_cast<half2&>(ret(retRow, n)) =
            __float22half2_rn(float2{data[0], data[1]});
      }
    }
  }
  return ret;
...
CUtensorMap makeTensorMapForQ(void const* addr, CUtensorMapDataType_enum dataType,
  ...
  // K26: upstream hardcoded SWIZZLE_64B because FP8 partElems=64 → 64 B row.
  // Under FP16, partElems=64 → 128 B row; use SWIZZLE_128B in that case.
  uint32_t const partBytes = partElems * elemBytes;
  auto const swizzle = (partBytes == 128) ? CU_TENSOR_MAP_SWIZZLE_128B
